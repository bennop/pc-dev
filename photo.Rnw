...
The view angle $\varphi$ for a regular (i.\,e., not fisheye) lens of focal length $f$ can be expressed as
\begin{equation}
\varphi = 2 \arctan \frac {d/2} f
\label{eq:viewangle}
\end{equation}
where $d$ is the size of the sensor. It is customary to use the sensor's diagonal (see column ``Diagonal" in Table~\ref{tab:sensor}) when giving the viewing angle for a lens.

This dependency is shown for a full-frame sensor and an APS-C sensor in  Fig.~\ref{fig:viewangle}.
<<viewangle, echo = FALSE, fig.cap = 'Viewing angle vs.\ focal length. In  the left the long side of a full-frame sensor is considered, on the right, both sides as well as the diagonal for full-frame  (black) and APS-C (grey) are shown.',fig.height = 6>>=
...
As  can be seen from the log-log-plot, the angle is inversely proportional to the focal length $f$ for longer $f$:
\begin{equation}
...
<<fscale, echo = FALSE, fig.cap = 'FOV scale vs.\ focal length. A  value of 1 means the FOV is as wide as the distance to the object, $D$. For the short edge FOV is reduced by 1.5 with respect to the long edge, for APS-C by 1.6 with respect to the full-frame sensor.',fig.height = 6>>=
#opar <- par(mfrow=c(1,2), mar = c(5,4,3,1) + .1)
curve(36/x,
      10,500,
...
\label{sec:coc}
(see \cite{cocwiki})
The CoC describes the blurring of a point source on the sensor. How
big a CoC is acceptable depends on the desired resolution, not
necessarily on the sensor but the resulting picture. For standard
viewing distances approx.\ 30\,$\mu$m is assumed for full-format
...
Getting the object even closer to the lens will move the image plane further out, but only dedicated macro lenses operate in that regime.

At $g=f$ the limit is reached where the image plane is pushed out to infinity, for $g<f$ no image can be formed (a \emph{vitual} image is formed on the object side of the lens.

Most lenses have a minimum focussing distance $g>2f$, given  by constructive constraints to how far the lens can be moved from the imaging plane (film or sensor). Macro lenses allow $2f\le g>f$.
...
Hyperfocal.plot()
@
<<HFDAPS-C, echo = FALSE, fig.cap = 'Canon APS-C sensor'>>=
...
<<nearfar85, echo = FALSE, fig.cap = 'Full format sensor',warning=FALSE,results='hide'>>=
...
<<dof85full, echo = FALSE, fig.cap = 'Full format sensor',warning=FALSE,results='hide'>>=
...
\end{eqnarray}
\subsection{Setting Depth}
\label{sec:setting-depth}

To achieve DOF between two distances $d_\mathrm{near}$ and
$d_\mathrm{far}$, adjust focussing distance and aperture to the values
given by
\begin{eqnarray}
  \label{eq:setdof}
  d & = & \frac{1}{2}\cdot\frac{1}{\frac{1}{d_\mathrm{near}} +
          \frac{1}{d_\mathrm{far}}}            \\
    & = & \frac{2d_\mathrm{near}d_\mathrm{far}}{d_\mathrm{near}+d_\mathrm{far}}
                                               \\
  F & = & \frac{f^2}{\COC}\cdot\frac{d_\mathrm{far} -
...
\label{sec:angle}
The viewing angle is determined by  the folcal length $f$ and
\cleardoublepage
\appendix
\section{DOF plots}
\label{apdx:DOFplots}
...
<<portrait24, echo=FALSE, fig.height=6, fig.width=6, fig.cap = paste0(24,'mm on full format sensor'), warning=FALSE,results='hide'>>=
full.dof.plot(24, 1.4, xlim=c(1,3), ylim = c(0.01,10), log="xy",  y.e=15)
@
\clearpage
\subsubsection{35mm}\label{sec:35}
<<portrait35, echo=FALSE, fig.height=6, fig.width=6, fig.cap = paste0(35,'mm on full format sensor'), warning=FALSE,results='hide'>>=
full.dof.plot(35, 1.4, xlim=c(1,3), ylim = c(0.01,10), log="xy",  y.e=15)
@
\clearpage
\subsubsection{50mm}\label{sec:50}
<<portrait50, echo=FALSE, fig.height=6, fig.width=6, fig.cap = paste0(50,'mm on full format sensor'), warning=FALSE,results='hide'>>=
full.dof.plot(50, 1.2, xlim=c(1,3), ylim = c(0.01,10), log="xy",  y.e=15)
@
\clearpage

\subsubsection{70mm}\label{sec:70}
<<portrait70, echo=FALSE, fig.height=6, fig.width=6, fig.cap = paste0(70,'mm on full format sensor'), warning=FALSE,results='hide'>>=
full.dof.plot(70, 1.2, xlim=c(1,3), ylim = c(0.01,10), log="xy",  y.e=15)
@
\clearpage

\subsubsection{85mm}\label{sec:85}
<<portrait85, echo=FALSE, fig.height=6, fig.width=6, fig.cap = paste0(85,'mm on full format sensor'), warning=FALSE,results='hide'>>=
full.dof.plot(85, 1.2, xlim=c(1,3), ylim = c(0.01,10), log="xy",  y.e=15)
@
\clearpage
\subsubsection{90mm}\label{sec:90}
<<portrait90, echo=FALSE, fig.height=6, fig.width=6, fig.cap = paste0(90,'mm on full format sensor'), warning=FALSE,results='hide'>>=
full.dof.plot(90, 1.2, xlim=c(1,3), ylim = c(0.01,10), log="xy",  y.e=15)
@
\clearpage

\subsubsection{100mm}\label{sec:100}
<<portrait100, echo=FALSE, fig.height=6, fig.width=6, fig.cap = paste0(100,'mm on full format sensor'), warning=FALSE,results='hide'>>=
full.dof.plot(100, 1.2, xlim=c(1,3), ylim = c(0.01,10), log="xy",  y.e=15)
@
\clearpage

\subsubsection{135mm}\label{sec:135}
<<portrait135, echo=FALSE, fig.height=6, fig.width=6, fig.cap = paste0(135,'mm on full format sensor'), warning=FALSE,results='hide'>>=
...
<<APS-Cportrait24, echo=FALSE, fig.height=7, fig.width=6, fig.cap = paste0(24,'mm on APS-C format sensor'), warning=FALSE,results='hide'>>=
full.dof.plot(24, 1.4, xlim=c(1,3), ylim = c(0.01,10), log="xy",  y.e=15, COC=0.018596)
@
\clearpage

\subsubsection{35mm APS-C}\label{secAPS-C35}
<<APS-Cportrait35, echo=FALSE, fig.height=6, fig.width=6, fig.cap = paste0(35,'mm on APS-C format sensor'), warning=FALSE,results='hide'>>=
full.dof.plot(35, 1.4, xlim=c(1,3), ylim = c(0.01,10), log="xy",  y.e=15, COC=0.018596)
@
\clearpage

\subsubsection{50mm APS-C}\label{secAPS-C50}
<<APS-Cportrait50, echo=FALSE, fig.height=6, fig.width=6, fig.cap = paste0(50,'mm on APS-C format sensor'), warning=FALSE,results='hide'>>=
full.dof.plot(50, 1.2, xlim=c(1,3), ylim = c(0.01,10), log="xy",  y.e=15, COC=0.018596)
@
\clearpage

\subsubsection{70mm APS-C}\label{secAPS-C70}
<<APS-Cportrait70, echo=FALSE, fig.height=6, fig.width=6, fig.cap = paste0(70,'mm on APS-C format sensor'), warning=FALSE,results='hide'>>=
full.dof.plot(70, 1.2, xlim=c(1,3), ylim = c(0.01,10), log="xy",  y.e=15, COC=0.018596)
@
\clearpage

\subsubsection{85mm APS-C}\label{secAPS-C85}
<<APS-Cportrait85, echo=FALSE, fig.height=6, fig.width=6, fig.cap = paste0(85,'mm on APS-C format sensor'), warning=FALSE,results='hide'>>=
full.dof.plot(85, 1.2, xlim=c(1,3), ylim = c(0.01,10), log="xy",  y.e=15, COC=0.018596)
@
\clearpage

\subsubsection{90mm APS-C}\label{secAPS-C90}
<<APS-Cportrait90, echo=FALSE, fig.height=6, fig.width=6, fig.cap = paste0(90,'mm on APS-C format sensor'), warning=FALSE,results='hide'>>=
full.dof.plot(90, 1.2, xlim=c(1,3), ylim = c(0.01,10), log="xy",  y.e=15, COC=0.018596)
@
\clearpage

\subsubsection{100mm APS-C}\label{secAPS-C100}
<<APS-Cportrait100, echo=FALSE, fig.height=6, fig.width=6, fig.cap = paste0(100,'mm on APS-C format sensor'), warning=FALSE,results='hide'>>=
full.dof.plot(100, 1.2, xlim=c(1,3), ylim = c(0.01,10), log="xy",  y.e=15, COC=0.018596)
@
\clearpage

\subsubsection{135mm APS-C}\label{secAPS-C135}
<<APS-Cportrait135, echo=FALSE, fig.height=6, fig.width=6, fig.cap = paste0(135,'mm on APS-C format sensor'), warning=FALSE,results='hide'>>=
