...
\author{compiled by \\[5mm]Benno Pütz\\\email{bpfoto@online.de}}
...
\label{sec:coc}
(see
\href{https://en.wikipedia.org/wiki/Circle_of_confusion}{Wikipedia})
...
curve(b(x,50),
      ...
      ylab = expression(paste("image distance ", italic(b), " [m]")),
      las  = 1)
grid()
curve(b(x,400),
      1, 200,
...
The hyperfocal distance (HFD) is the distance that when set on a lens results in the largest possible depth of field (DOF) for a given focal length/aperture combination: everything between a minumum distance of $\mathrm{HFD}/2$ to $\infty$ is \emph{in focus} (i.\,e., not more blurred than \COC).

The HFD depends on focal length, aperture, and the
desired resolution (COC)
\begin{eqnarray}
  \label{eq:HFD}
...
\label{sec:dof}
(see
\href{https://www.largeformatphotography.info/articles/IntroToDoF.pdf}{DOF.pdf})
...
\item approximation by  single lens \item lens imperfections (spherical and chromatic aberration)\end{itemize}} consider an object at infinity ($g\rightarrow\infty$): the rays enter the lens in parallel and are focussed in a plane with distance $b=f$ behind the lens.
...
At $g=f$ the limit is reached where the image plane is pushed out to infinity, for $g<f$ no image can be formed.
...
For an arbitrary object distance $g>2f$
