...
At $g=f$ the limit is reached where the image plane is pushed out to infinity, for $g<f$ no image can be formed (i.\,e., no \emph{real image}, a \emph{vitual} image is formed on the object side of the lens).

Most lenses have a minimum focussing distance $g>2f$, given  by constructive constraints as to how far the lens can be moved from the imaging plane (film or sensor). Macro lenses allow $2f\le g>f$.
